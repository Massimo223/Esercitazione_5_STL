...
struct PolygonalMesh
{
    ...
    std::map<unsigned int, list<unsigned int>> MarkerCell1Ds = {}; ///< Cell1D markers

    unsigned int NumCell2Ds = 0; ///< number of Cell2D
    std::vector<unsigned int> Cell2DsId = {}; ///< Cell2D id, size 1 x NumberCell2D
    std::vector<std::vector<unsigned int>> Cell2DsVertices = {}; ///< Cell2D Vertices indices, one vector per polygon
    std::vector<std::vector<unsigned int>> Cell2DsEdges = {}; ///< Cell2D Edges indices, one vector per polygon
